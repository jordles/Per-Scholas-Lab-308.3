...
for (let i = 1; i <= 100; i++) {
  ...
  console.group("Part 2: Prime Time");
...
console.group("Part 3: Feeling Loopy");
...
//TABLE SOLUTION USING CONSOLE.TABLE
  let headers = [];
  let rows = [];
  let cell = '';
  let row = [];
  let isHeader = true; // Flag to differentiate between headers and rows
  
  for (let i = 0; i <= csv.length; i++) {
    let char = csv[i] || '\n'; // Treat end of string like a newline to capture last row; so if undefined become '\n'
    if (char === ',') {
      row.push(cell);
      cell = '';
      continue;
    ...
    if (char === '\n') {
      ...
      if (isHeader) {
        headers = row;  // Set headers on first row
        isHeader = false;
      } else {
        let rowObj = {};
...
console.table(rows); // Output table
